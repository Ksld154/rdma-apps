...
static void on_completion(struct ibv_wc *wc) {
    ...
    if(wc->opcode & IBV_WC_RECV && ctx->msg->id == MSG_MR) {
        ctx->peer_addr = ctx->msg->data.mr.addr;
        ctx->peer_rkey = ctx->msg->data.mr.rkey;

        ctx->payload_idx = 0;
        printf("received MR, sending chunk #%d\n", ctx->payload_idx);

        send_data_chunk(id, ctx->payload_idx++);

    } else if(wc->opcode & IBV_WC_RECV && ctx->msg->id == MSG_READY) {
        uint32_t result = ntohl(wc->imm_data);
        printf("Compute result: %u\n", result);
        printf("received READY, sending chunk #%d\n", ctx->payload_idx);

        usleep(100);
        send_data_chunk(id, ctx->payload_idx++);

    } else if(wc->opcode & IBV_WC_RECV && ctx->msg->id == MSG_DONE) {
